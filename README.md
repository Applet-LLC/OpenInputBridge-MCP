...
- **マウスの絶対移動(`absolute:true`)は、実機検証が中断・保留中で信頼性未確認です**。大きく異なる正規化座標を送ってもカーソルがほぼ同じ位置にしか着地しない現象を観測しましたが、検証中にディスプレイ構成が(利用者の操作によらず)デュアルモニタからシングルモニタへ変化したタイミングと重なっており、原因を確定できていません。マルチモニタ環境で再検証できるまでは、`mouse_click`を含め**絶対座標指定は使用せず、相対移動(`absolute:false`)を使うことを推奨**します(詳細は [test/REALWORLD_TESTING.md](test/REALWORLD_TESTING.md) の項目6)
- 同様の理由で、`mouse_click`のクリック位置精度も未検証です(ボタン押下自体はエラーなく実行されます)
- **Windows専用**
...
- **マウス絶対移動(`absolute:true`)・`mouse_click`の座標精度検証(保留中)**: マルチモニタ環境を再現できるようになり次第、安定したディスプレイ構成下で再実施する
